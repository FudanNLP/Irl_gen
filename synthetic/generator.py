...
class Generator(object):
    def __init__(self, num_emb, batch_size, emb_dim, hidden_dim,
        ...
        # self.Wi = tf.Print(self.Wi, [self.Wi[1,:10]], message='generator.wi======',summarize=100)
        ...
        # self.off_policy_dist = tf.placeholder(tf.float32, shape=[self.batch_size, self.sequence_length, self.num_emb], name='off_policy_dist')
        self.rewards = tf.placeholder(tf.float32, shape=[self.batch_size, self.sequence_length], name='aaa') # get from rollout policy and discriminator
       ...
       # sig_bz_log_pred = tf.nn.sigmoid(tf.reshape(log_pred, [self.batch_size, self.sequence_length]))
        ...
        # self.g_loss = -tf.reduce_mean(log_pred * tf.reshape(self.rewards - accumlated_pred - 1, [-1]))
        # weighted_log_pred = tf.reduce_sum(self.clipped_log_pred, 1) * tf.reshape(self.gamma, [-1])
    ...
    def pretrain_step(self, sess, x):
        outputs = sess.run([self.pretrain_updates, self.pretrain_loss], feed_dict={self.x: x, self.learning_rate: 0.005})
        return outputs

    def rl_train_step(self, sess, x, rewards, baseline, offpolicy, decay_weight, learn_rate):
        outputs = sess.run([self.g_updates, self.g_loss], feed_dict={self.x:x, self.rewards: rewards,
                                                                     ...
                                                                     self.decay_weight: decay_weight, self.learning_rate: learn_rate})
        return outputs
